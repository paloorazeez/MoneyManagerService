...
public class PayableRepositoryImpl implements PayableRepositoryCustom {
    ...
    public Map<String, Object[]> getSummaryByPayTo(String userId, Date dateFrom, Date dateTo) {
        ...
        MatchOperation matchStage = Aggregation.match( Criteria.where("user.$id").is(new ObjectId(userId)).and("paymentDate").gte(dateFrom).lte(dateTo) );
        GroupOperation groupingStage = Aggregation.group("payTo").sum("amount").as("totalAmount");

        Aggregation aggregation = Aggregation.newAggregation( matchStage, groupingStage);

        AggregationResults<Document> expense = primaryMongoTemplate.aggregate(aggregation, "payable", Document.class);
        if(expense != null) {
    ...
    public List<String> getPayToList(String userId) {
        ...
        List<String> categoryList = primaryMongoTemplate.findDistinct(query, "payTo", Payable.class, String.class);
